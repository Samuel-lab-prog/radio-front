	...
	return (
		<div className='min-h-screen bg-[#f5f7fa] pb-28 text-[#10213b]'>
			<header className='bg-[#071120] text-[#f6f8fc]'>
				<div className='relative mx-auto flex min-h-[74px] w-[calc(100%-2rem)] max-w-[1120px] flex-wrap items-center gap-x-10 gap-y-3 py-3'>
					<Link
					...
					<nav
						...
						className={`order-3 w-full flex-col gap-1 overflow-hidden rounded-xl border border-white/10 bg-white/5 p-1 text-xs text-[#bfd0e6] transition-[max-height,opacity,transform] duration-200 ease-out sm:absolute sm:left-1/2 sm:top-1/2 sm:order-none sm:flex sm:max-h-none sm:w-auto sm:-translate-x-1/2 sm:-translate-y-1/2 sm:flex-row sm:items-center sm:overflow-visible sm:text-sm sm:opacity-100 sm:pointer-events-auto ${menuOpen ? 'pointer-events-auto flex max-h-80 translate-y-0 opacity-100' : 'pointer-events-none flex max-h-0 -translate-y-2 opacity-0'}`}
						id='main-navigation'
					>
						<NavLink
							className={({ isActive }) =>
